...
const $searchResult = document.querySelector(".autocomplete-search-box .search-result");
const loader = {
...
document.querySelector(".title").innerText = `Medication code set creator v${version}`;
...
function onWorkerMessage(e) {
  ...
  switch (msg) {
    ...
    case "loaded":
      ...
      loader[terminology].querySelector(".lds-facebook").style.visibility = "hidden";
      const termLoader = loader[terminology].querySelector(".terminology-loader");
...
function elHeight(el) {
  ...
  return el.offsetHeight + parseFloat(styles["margin-top"]) + parseFloat(styles["margin-bottom"]);
}
...
$extraTerms.addEventListener("click", async (e) => {
  if (e.target.dataset && e.target.dataset.word) {
    ...
    if (e.target.classList.contains("info-button")) {
      ...
      const mainWordCapped = mainWord[0].toUpperCase() + mainWord.slice(1).toLowerCase();

      const promises = [];

      promises.push(
        cachedResponses.rxNorm[term] ||
          fetch(`https://rxnav.nlm.nih.gov/REST/approximateTerm.json?term=${term}`).then((x) =>
            x.json()
          )
      );
      ...
      if (
        ...
        dbPediaResponse.results.bindings
          .map((x) => x.z.value.split("/").reverse()[0].split("_").join(" ").toLowerCase())
      ...
      if (
        ...
        const possibleMatches = rxNormResponse.approximateGroup.candidate
          .map((x) => x.name || false)
          .filter((x) => x && x.toLowerCase().indexOf($wordInput.value.trim().toLowerCase()) > -1);
      ...
      if (msg.length > 0) {
        ...
        window.open(`https://www.google.com/search?q=${e.target.dataset.word}`, "_blank");
      }
    } else {
  ...
  $extraTerms.innerHTML = Object.keys(includedExtraWords)
    ...
    .map(
      (word) =>
        ...
        }">${includedExtraWords[word] ? "Exclude" : "Include"}</button></div></div>`
    )
    .join("");
}
...
function displayResults() {
  if (data[selectedTerminology].concepts) {
    ...
    const originalLength = Object.entries(data[selectedTerminology].concepts).length;
    ...
    // Excluded codes
    const excludedItems = Object.entries(data[selectedTerminology].excludedConcepts);
    ...
    $message.innerText = (data && data.snomed && data.snomed.message) || `No matches`;
    displayExtraWords([]);
...
$wordInput.addEventListener("keydown", (event) => {
  const callback = {
    ...
    ArrowUp: () => {
      ...
      const currentlyHighlighted = $searchResult.querySelector(".highlighted");
      if (!currentlyHighlighted) {
        if ($searchResult.lastChild) $searchResult.lastChild.classList.add("highlighted");
      } else {
    ...
    ArrowDown: () => {
      ...
      const currentlyHighlighted = $searchResult.querySelector(".highlighted");
      if (!currentlyHighlighted) {
        if ($searchResult.firstChild) $searchResult.firstChild.classList.add("highlighted");
      } else {
        const children = $searchResult.children;
        const index = [...children].indexOf(currentlyHighlighted);
