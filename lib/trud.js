...
import { ensureDir, getDirName, heading } from './utils.js';
...
async function extractDataFromZip(
  zipFilePath,
  zipFileRegexArray,
  outDir,
  isDontExtractPath
  ...
  await new Promise((resolve) => {
    createReadStream(zipFilePath)
      ...
      .on('entry', function (entry) {
        if (
          zipFileRegexArray.filter((regex) =>
...
async function downloadAndExtractBNFFiles() {
  ...
  const hierarchyFiles = readdirSync(HIER_DIR).filter(
    (x) => x.indexOf('Hierarchy') > -1
  );
  if (!hierarchyFiles || hierarchyFiles.length === 0) {
    ...
    const { fileName, dateString, date } = hierarchyFiles
      .map((fileName) => {
        ...
        if (!dateString.match(/^[0-9]{8}$/)) {
          console.log(
            '> There is a BNF hierarchy file that is not of the format BNF-Hierarchy-YYYYMMDD.csv'
          );
          process.exit();
        }
        const date = new Date(
    ...
    const currentYearMonth = `${new Date().getFullYear()}${`0${
      new Date().getMonth() + 1
    }`.slice(-2)}`;

    if (dateString.indexOf(currentYearMonth) === 0) {
      console.log('> Already got the latest BNF hierarchy.');
    } else {
      console.log(
    ...
    writeFileSync(
      join(BNF_DIR, 'latest-hierarchy.json'),
  ...
  const html = await response.text();
  const urls = html
    .match(/\/sites\/default\/files\/[0-9-]+\/BNF[^"]+zip/g)
    .map((zipFileUrl) => {
      const [, zipFileNameRaw] = zipFileUrl.match(/\/([^/]+.zip)/);
      const zipFileName = zipFileNameRaw
        .replace(/%20/g, '-')
        .replace(/_[0-9].zip/, '.zip');
  ...
  if (existingFiles.indexOf(zipFileName) > -1) {
    console.log(
      `> The latest zip file for 'BNF mapping' already exists so no need to download again.`
    );
  } else {
    console.log(
      `> The latest zip file for 'BNF mapping' is not stored locally. Downloading...`
    );
    const outputFile = join(ZIP_DIR, zipFileName);
